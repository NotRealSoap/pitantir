 ...
 *
 * Usage:
 *   DATABASE_URL=postgresql://... npx pnpm@10.11.0 --filter @pitantir/worker exec tsx src/dump-scan-item.ts
 *   DATABASE_URL=... npx pnpm@10.11.0 --filter @pitantir/worker exec tsx src/dump-scan-item.ts "billionaire"
 *   DATABASE_URL=... npx pnpm@10.11.0 --filter @pitantir/worker exec tsx src/dump-scan-item.ts "lifesteal" --all
 */
...
const payload = {
  ...
  note:
    "Stored shape is book-oriented (title/author/pages/nonce + hypixelExtraAttributes). Enchant names like Billionaire/Lifesteal usually live in ExtraAttributes.CustomEnchants or display lore — older scans may only have ExtraAttributes if lore was not persisted.",

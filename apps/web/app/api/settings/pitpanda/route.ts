...
import {
  getPitPandaApiKey,
  isPitPandaConfigured,
  setPitPandaApiKey,
} from "../../../../src/server/pitpanda-key-store";
...
export async function GET() {
  return NextResponse.json({
    configured: isPitPandaConfigured(),
    provider: "pitpanda",
  });
}
...
export async function POST(request: Request) {
  ...
  try {
    await setPitPandaApiKey(apiKey);
    // Confirm without echoing secrets
    return NextResponse.json({
      ok: true,
      configured: Boolean(getPitPandaApiKey()),
    });
  } catch (error) {
